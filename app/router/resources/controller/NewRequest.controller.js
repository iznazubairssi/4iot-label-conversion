sap.ui.define([
    ...
    return Controller.extend("label.conversion.project1.controller.NewRequest", {
        ...
        onRouteMatched: function () {
            var sEmail = sessionStorage.getItem("userEmail");
        ...
        onSubmitRequest: async function () {
            ...
            // Check for existing request using /api/requests
            try {
                const response = await fetch(`/api/requests/${encodeURIComponent(oFormData.contactMail)}`);
                const data = await response.json();
                
                if (data.value && data.value.length > 0) {
                    MessageBox.error(
                        "A request already exists for this email address.\n\n" +
                        "Request ID: " + data.value[0].ID + "\n\n" +
                ...
                // No existing request, proceed with upload
                that.uploadFilesWithXHR(aFiles, oFormData);
                
            } catch (e) {
                console.error("Error checking for existing request:", e);
